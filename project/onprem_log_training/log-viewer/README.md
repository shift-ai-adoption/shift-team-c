...
| # | 画面 | URL | 役割 |
|---|---|---|---|
| 1 | **RoboMart（ECサイト）** | `http://<研修環境のIP>:3002/` | ログを発生させる側。商品を閲覧・操作するとログが出力されます |
| 2 | **Log Viewer（ダッシュボード）** | `http://<研修環境のIP>:8001/` | ログを可視化する側。①の操作結果がここに表示されます |

> **ポート番号（`3002` と `8001`）はそのまま使ってください。**
> `<研修環境のIP>` の部分だけを、研修担当者からお知らせするIPアドレスに置き換えます。
> ご自身のPC上で起動している場合は `localhost` に読み替えてください
> （例: `http://localhost:3002/`、`http://localhost:8001/`）。
...
ブラウザで `http://<研修環境のIP>:8001/` を開きます（Cチーム研修環境のポートは **8001**）。
公開ポートは `.env` の `LOG_VIEWER_PORT` で指定してください（環境のSG/ファイアウォールで開放済みのポートに合わせること）。
コンテナ内部は常に `8090` で動作し、`8001->8090` のように公開ポートへマッピングされます。
